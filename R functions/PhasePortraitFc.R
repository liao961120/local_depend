...
# Rob de Boer, Utrecht University, 10-10-2017
...
plane <- function(xmin=0, xmax=1.1, ymin=0, ymax=1.1, log="", npixels=500, state=s, parms=p, odes=model, x=1, y=2, time=0, grid=5, eps=0, show=NULL, portrait=FALSE, vector=FALSE, add=FALSE, legend=TRUE, zero=TRUE, lwd=2, ...) {
  ...
  if (!add) {
    do.call('plot',c(list(type='n',range(xmin,xmax),range(ymin,ymax),xlab=xvar,ylab=yvar,log=log,font.main=font.main,font.sub=font.sub),dots[names(dots) %in% args_plot]))
    if (legend)
    ...
    for (i in seq(1,grid)) {
      ...
      for (j in seq(1,grid,1)) {
        ...
        if (portrait) {
          ...
          nsol <- do.call('run',c(list(state=state,parms=parms,timeplot=FALSE,table=TRUE),dots_run))
          lines(cbind(nsol[x+1],nsol[y+1]))
        }
        if (vector) {
...
run <- function(tmax=100, tstep=1, state=s, parms=p, odes=model, ymin=0, ymax=NULL, log="", x=1, y=2, xlab="Time", ylab="Density", tmin=0, draw=lines, times=NULL, show=NULL, arrest=NULL, after=NULL, tweak=NULL, timeplot=TRUE, traject=FALSE, table=FALSE, add=FALSE, legend=TRUE, solution=FALSE, delay=FALSE, lwd=2, ...) {   
  ...
  if (solution) {                              # Run in one go
    ...
    if (is.null(after)){                       # Run in one go 
      ...
      for (t in times[2:length(times)]) {
        ...
        else nsol <- rbind(nsol,nsol0[2,])
        state <- nsol0[2,2:(nvar+1)]
        if (!is.null(after) & t %in% times) eval(parse(text=after))
        t0 <- t
      }
  ...
  if (traject) {
    points(state[x],state[y],pch=20)
    lines(nsol[,x+1],nsol[,y+1],lwd=lwd)
  }
...
continue <- function(state=s, parms=p, odes=model, x=1, step=0.01, xmin=0, xmax=1, y=2, ymin=0, ymax=1.1, log="", time=0, positive=FALSE, add=FALSE, ...) {  
  ...
  if (attr(q0,"steady")) {
    cat("Starting at",names(p[x]),"=",p[x],"with:\n")
    print(q0$y)
    bary <- q0$y[y]
    ...
    if (!add)
      do.call('plot',c(list(range(xmin,xmax),range(ymin,ymax),type='n',xlab=names(p0),ylab=names(state)[y],log=log,font.main=font.main,font.sub=font.sub),dots[names(dots) %in% args_plot]))
...
fit <- function(datas=data, state=s, parms=p, odes=model, free=NULL, who=NULL, differ=NULL, fixed=NULL, tmin=0, tmax=NULL, ymin=NULL, ymax=NULL, log="", xlab="Time", ylab="Density", bootstrap=0, show=NULL, fun=NULL, costfun=cost, initial=FALSE, add=FALSE, timeplot=TRUE, legend=TRUE, main=NULL, sub=NULL, pchMap=NULL, ...) {
  ...
  for (iset in seq(nsets)) {
    ...
    if (initial) {
      ...
      state <- unlist(data[1,2:ncol(data)])
    }
...
timePlot <- function(data, tmin=0, tmax=NULL, ymin=0, ymax=NULL, log="", xlab="Time", ylab="Density", show=NULL, legend=TRUE, draw=lines, lwd=2, add=FALSE, main=NULL, sub=NULL, recolor=FALSE, colMap=NULL, pchMap=NULL, ...) {
  ...
  if (is.null(tmax)) tmax <- data[nrow(data),1]
  if (is.null(ymax)) ymax <- max(data[,2:ncol(data)])
  if (tmin == 0 & grepl('x',log)) tmin <- data[1,1]
  if (ymin == 0 & grepl('y',log)) ymin <- min(data[,2:ncol(data)])
  if (!add)
    plot(range(tmin,tmax),range(ymin,ymax),log=log,type='n',xlab=xlab,ylab=ylab,main=main,sub=sub,font.main=font.main,font.sub=font.sub,...)
  colnames <- names(data)[2:ncol(data)]
